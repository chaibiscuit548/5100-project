...
Just to confirm these results, lets do some pearson correlation tests

```{r}
coneduc_spend <- coneduc_nipa %>%
  filter(question == "coneduc",
         response == "HARDLY ANY") %>%
  dplyr::select(year, prop, gved_pc_y) %>%
  drop_na()

nateduc_spend <- nateduc_nipa %>% 
  filter(question == "nateduc",
         response == "TOO LITTLE") %>%
  dplyr::select(year, prop, gved_pc_y) %>%
  drop_na()

cor.test(coneduc_spend$gved_pc_y,coneduc_spend$prop,method = "pearson")
cor.test(nateduc_spend$gved_pc_y,nateduc_spend$prop,method = "pearson")
